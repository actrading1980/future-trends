...
**Estado (2026-07-02):** Etapa 1 (selección mecánica del universo ampliado) completada — ver `data/universe_selection_20260702.json`. Resultado real: 136 empresas (51 + 85 nuevas), no 150 — Semiconductores y Clean Energy quedaron bajo cuota por pool insuficiente tras filtros de dollar ADV y presentación de 10-K, sin redistribución entre categorías. Ver `validation_engine_v1.1.md` Sección 6.2 para el recálculo de potencia con N real.

**Requisito de seguridad para Etapa 3 (extracción LLM de 10-Ks), pre-registrado antes de empezar:** el prompt del extractor debe tratar el contenido de los filings **siempre como datos, nunca como instrucciones**. El pipeline va a leer 136 documentos de terceros con un LLM — un 10-K o un Item 1A con texto que se parezca a una instrucción (inyectado deliberadamente o por accidente de formato) no debe alterar el comportamiento del extractor. Motivación concreta, no hipotética: durante la Etapa 1 apareció una línea con forma de instrucción ("Instruction: Change of selection promptly shifts...") en un fragmento de página web pegado en la conversación — inofensiva en ese contexto porque un humano la filtró antes de que llegara a ejecutarse, pero el pipeline de Etapa 3 no tiene ese filtro humano intermedio por documento. El prompt de `company_update.md` (Sección 6.2 del spec) ya sigue el patrón correcto (genera diff, nunca escribe en la DB); extender esa misma disciplina a "el texto del filing no es un canal de comandos" en la Etapa 3.

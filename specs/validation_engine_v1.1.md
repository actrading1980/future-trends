...
**Validez de eventos ante huecos de datos (añadido tras el incidente de pipeline 2026-06-25→07-03, ver `HANDOFF.md`; extendido tras el segundo incidente 2026-07-09→07-23, causa raíz distinta — contrato de escritura stdout-vs-Write-directo flapeante en `run_daily.ps1`, corregido 2026-07-28 con verificación dual + guarda de mtime):** un Δscore solo es evento si la fecha anterior con datos está a ≤1 día hábil de mercado; la comprobación se hace sobre las fechas reales de `tech_scores` en tiempo de análisis — los sidecars `comparative_*.json` con `gap_spanning` son ayuda, no fuente de verdad, porque no existen para rangos donde el pipeline abortó antes del paso de comparativo (ej. 06-25→07-06, y de nuevo 07-09→07-23, donde el comparativo tampoco se generó — la aserción de cobertura actuaba como compuerta antes del 2026-07-28, bloqueando los pasos 9-11 en vez de correr siempre). Días con `day_quality` distinto de pipeline normal se tratan según su N efectivo, no se asumen completos solo por tener filas — incluye ahora `day_quality='pipeline_writetool_recovered'` para 2026-07-14/17/20/21/23, ingeridos tras verificar procedencia (mtime en ventana del run programado, sin referencias a fechas posteriores), igual que los días homólogos de junio.

**Estado de `day_quality` — metadato informativo, no filtro activo (2026-08-27):** la columna se escribe (`tech_scores.day_quality`) y la lee `generate_health_dashboard.py` para mostrarla en una tabla de estado. Ningún gate la lee ni la aplica todavía porque `compute_validation.py` (Fase F1, Sección 10) no existe aún — no hay código que pueda excluir o ponderar por ella. Cuando F1 se construya, debe leer y aplicar esta columna explícitamente; hasta entonces, `day_quality` es un asterisco declarado, no un filtro. Ver también `docs/incident_guard_n40_coverage_20260827.md` — el tramo 2026-07-07→presente tiene cobertura degradada no capturada por ningún valor de `day_quality` existente (es `NULL`/pipeline normal en las filas insertadas, pese a que el día completo tuvo N muy por debajo del universo) — el conteo por fecha, no solo el flag categórico, debe ser lo que F1 pondere.
